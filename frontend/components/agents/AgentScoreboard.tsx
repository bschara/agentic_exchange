    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-2 max-h-[280px] overflow-y-auto">
        {ranked.map((agent, idx) => {
          ...
          const hasPnl =
            Math.abs(realizedPnl) > 0.01 || Math.abs(unrealizedPnl) > 0.01;

          return (
            <div
              key={agent.agent_id}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-all duration-300 ${pnlBg(
                pnl
              )}`}
            >
              {/* Rank */}
              <span className="text-base w-5 text-center flex-none">
                {MEDALS[idx] ?? `${idx + 1}.`}
              </span>

              {/* Agent identity */}
              <span className="text-base flex-none">
                {agent.icon || AGENT_ICONS[agent.agent_id] || "🤖"}
              </span>
